...
public record Action() implements backend.api.app.Action {
	...
	public void get(Application app, ResponseData res, Connection db, String id, String token) {
		if(!id.equals(RequestData.INVALID)) {
			try {
				var action = backend.api.app.utils.Action.action(db, app, id);
				...
				if(action == null) {
					...
					var history = backend.api.app.utils.Action.history(db, app, action.name());
			...
			try {
				res.addArray("actions", backend.api.app.utils.Action.actions(db, app).stream().map(action -> action.toMap(app)).toList());
				res.send(200);
